...
public final class Turret extends Unit implements ITinkable {
    ...
    public IntVector2D findAttackingPointOrNull(ArrayList<Unit> units) {
        ...
        if (targets.size() > 1) {
            ...
            for (int i = 0; i < targets.size() && isContinue; i++) {
                if (targets.get(i).position.getY() == this.position.getY() && targets.get(i).position.getX() == this.position.getX()) {
            ...
            for (int i = 0; i < targets.size() && isContinue; i++) {
                if (targets.get(i).position.getY() == this.position.getY() - 1 && targets.get(i).position.getX() == this.position.getX()) {
                    targets.add(targets.get(i));
                    isContinue = false;
                    break;
                }
            ...
            for (int i = 0; i < targets.size() && isContinue; i++) {
                if (targets.get(i).position.getY() == this.position.getY() - 1 && targets.get(i).position.getX() == this.position.getX() + 1) {
                    targets.add(targets.get(i));
                    isContinue = false;
                    break;
                }
            ...
            for (int i = 0; i < targets.size() && isContinue; i++) {
                if (targets.get(i).position.getY() == this.position.getY() && targets.get(i).position.getX() == this.position.getX() + 1) {
                    targets.add(targets.get(i));
                    isContinue = false;
                    break;
                }
    ...
    public boolean isAttackable(Unit unit) {
        ...
        if (this.position.equals(unit.getPosition())) {
            ...
            return true;
        } else if (this.position.getX() + 1 == unit.position.getX() && this.position.getY() - 1 == unit.position.getY()) {
            return true;
